...
const ContactMe = () => {
  ...
  /* --------------------------------------------
      SAFE GSAP ANIMATION — NO NULL TARGETS EVER
  ---------------------------------------------*/
  useEffect(() => {
    if (
      ...
      console.warn("GSAP prevented: Missing refs.");
      return;
    }
    ...
    const safeFrom = (target, vars) => {
      ...
      if (!elements || elements.length === 0 || !elements[0]) return;
      gsap.from(elements, vars);
    };

    const ctx = gsap.context(() => {
      safeFrom(containerRef.current, {
        opacity: 0,
        y: 50,
        duration: 0.8,
        ease: "power3.out",
      ...
      safeFrom(gsap.utils.toArray(".char"), {
        y: "random(-50,50)",
        opacity: 0,
        duration: 0.8,
        ease: "back.out(1.7)",
  ...
  const sendEmail = (e) => {
    ...
    if (!serviceId || !templateId || !publicKey) {
      console.error("❌ ENV NOT LOADED — Check Vercel Env Variables");
      setSubmitStatus("error");
      setIsSubmitting(false);
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="bg-element absolute top-1/4 left-1/4 w-32 h-32 rounded-full bg-indigo-100/20 dark:bg-indigo-900/10 blur-3xl"></div>
        <div className="bg-element absolute bottom-1/3 right-1/4 w-40 h-40 rounded-lg bg-teal-100/20 dark:bg-teal-900/10 blur-3xl"></div>
      </div>

      <div className="container mx-auto px-4 md:px-6 max-w-6xl relative z-10">
        {/* Heading */}
        <div ref={headingRef} className="text-center mb-10 md:mb-16">
          <h2 className="text-3xl md:text-6xl font-bold mb-4 dark:text-white">
            {renderAnimatedTitle("Get In Touch")}
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div
            ref={formRef}
            className="bg-white/80 dark:bg-gray-800/90 backdrop-blur-sm rounded-2xl p-8 shadow-lg"
          >
            <form ref={form} onSubmit={sendEmail} className="space-y-6">
              <div className="form-element">
                <label className="text-sm font-medium">Your Name</label>
                <input name="user_name" required className="input" />
              </div>

              <div className="form-element">
                <label className="text-sm font-medium">Email</label>
                <input name="user_email" required type="email" className="input" />
              </div>

              <div className="form-element">
                <label className="text-sm font-medium">Message</label>
                <textarea name="message" required className="input h-32"></textarea>
              </div>

              <button
                type="submit"
                disabled={isSubmitting}
                className="px-6 py-3 bg-indigo-500 text-white rounded-lg shadow"
              >
                {isSubmitting ? "Sending..." : "Send Message"}
              </button>

              {submitStatus === "success" && (
                <p className="success-message text-green-600 text-sm">Message sent!</p>
              )}
              {submitStatus === "error" && (
                <p className="error-message text-red-600 text-sm">Failed to send message.</p>
              )}
            </form>
          </div>
          ...
          <div ref={infoRef} className="space-y-6">
            <div className="contact-info-item">
              ...
              <a href="mailto:shreyashpatil8078@gmail.com">contact@example.com</a>
            </div>
          </div>

          {/* SOCIAL */}
          <div ref={socialRef} className="space-y-4">
            <a className="social-icon" href="https://github.com">GitHub</a>
            <a className="social-icon" href="https://linkedin.com">LinkedIn</a>
          </div>
        </div>
      </div>
    </section>
